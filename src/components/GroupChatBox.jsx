    ...
    <Card className="shadow-lg" style={{ minHeight: '470px' }}>
      ...
      <Card.Body style={{ maxHeight: '470px', overflowY: 'auto' }}>
        <ListGroup variant="flush">
          {messages.map((msg) => (
            <div key={msg.id} className={`d-flex mb-1 ${msg.senderId === userId ? 'justify-content-end' : 'justify-content-start'}`}>
              <ListGroup.Item
                className={`rounded px-3 py-2 mb-1 ${msg.senderId === userId ? 'bg-light text-end' : 'bg-primary text-white text-start'}`}
